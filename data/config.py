...
class Sheets:
    ...
    수정원가기준손익_메모 = (SheetID.수정원가기준손익, "8. 수정원가기준_손익_별도_메모")
    ...
    수익성_메모_연결 = (SheetID.수익성, "16. 수익성_연결_메모_DB")
    수익성_대표이사_DB = (SheetID.수익성, "16-1. 수익성_대표이사_DB")
    수익성_대표이사_메모 = (SheetID.수익성, "16-1. 수익성_대표이사_메모")
    ...
    # 20. 전월대비손익차이
    전월대비손익차이_DB = (SheetID.전월대비손익차이, "전월대비 손익차이_DB")
    전월대비손익차이_메모 = (SheetID.전월대비손익차이, "전월대비_손익차이_메모")

    # 21. 수출환율차이
    수출환율차이_DB = (SheetID.수출환율차이, "수출 환율 차이_DB")
    ...
    # 26. 제조 가공비
    제조가공비_DB = (SheetID.제조가공비, "제조가공비_DB")
    제조가공비_메모 = (SheetID.제조가공비, "제조가공비_메모")

    # 27.판매비와 관리비
    판매비와관리비_DB = (SheetID.판매비와관리비, "판매비와 관리비_DB")
    ...
    # 5. 비용분석 43~49
    부재료사용량_DB = (SheetID.부재료사용량, "부재료 사용량 원단위 추이")
    부재료사용량_포항_메모 = (SheetID.부재료사용량, "부재료 사용량 원단위 추이_메모_포항")
    부재료사용량_충주_메모 = (SheetID.부재료사용량, "부재료 사용량 원단위 추이_메모_충주")
    부재료사용량_충주2_메모 = (SheetID.부재료사용량, "부재료 사용량 원단위 추이_메모_충주2")
    
    부재료단가추이_DB = (SheetID.부재료단가추이, "부재료단가추이")
    부재료단가추이_메모 = (SheetID.부재료단가추이, "부재료 단가 추이_메모")
    월평균클레임_DB =(SheetID.비용, "47. 월 평균 클레임 지급액_DB")
    당월클레임_메모 = (SheetID.비용, "48. 당월 클레임 내역_메모")
    ...
    연령별재고현황_메모 = (SheetID.재고, "51~54. 연령별 재고현황_메모")
    총재고_메모 = (SheetID.재고, "54. 총 재고 및 장기재고 현황_메모")
    ...
    산업군별영업이익_메모 = (SheetID.유형별손익, "96. 산업군별 영업이익_메모")
    실수요유통영업이익_DB = (SheetID.유형별손익, "97. 실수요유통 영업이익_DB")
    실수요유통영업이익_메모= (SheetID.유형별손익, "97. 실수요유통 영업이익_메모")
    메이커별영업이익_DB = (SheetID.유형별손익, "98. 메이커별 영업이익_DB")
    메이커별영업이익_메모 = (SheetID.유형별손익, "98. 메이커별 영업이익_메모")
    부서메이커별영업이익_DB = (SheetID.부서별유형별손익, "99. 부서메이커별 영업이익_DB")
    부서메이커별영업이익_메모 = (SheetID.부서별유형별손익, "99. 부서메이커별 영업이익_메모")
    부서사업장메이커별영업이익_DB = (SheetID.부서별유형별손익, "100. 부서사업장메이커별 영업이익_DB")
    부서사업장메이커별영업이익_메모 = (SheetID.부서별유형별손익, "100. 부서사업장메이커별 영업이익_메모")
    부서별인당영업이익_DB = (SheetID.부서별유형별손익, "101. 부서별 인당 영업이익_DB")
    부서별인당영업이익_메모 = (SheetID.부서별유형별손익, "101. 부서별 인당 영업이익_메모")

선재_국내_사업장 = '본사'

# 1,6 손익
CORP_ORDER      = ['선재_국내', '선재_중국', '선재_타이']  # 손익 사업장 순서

# 2. 현금흐름표(연결)
현금_CORP_ORDER     = ['전체', '본사', '남통', '태국']                    # 현금흐름표 DB 구분2 순서
현금_사업장_표시명  = {'전체': '선재_전체','본사': '선재_국내', '남통': '선재_중국', '태국': '선재_태국'}
...
재무_CORP_ORDER      = ['선재_국내', '선재_중국', '선재_태국']          # 재무상태표 DB 사업장 순서
재무_사업장_표시명   = {'특수강': '선재_국내', '남통': '선재_중국', '태국': '선재_태국'} 

# 4. 현금흐름표(연결)
회전일_CORP_ORDER     = ['계', '특수강', '남통', '태국']                    # 현금흐름표 DB 구분2 순서
회전일_사업장_표시명  = {'계': '선재_전체', '특수강': '선재_국내', '남통': '선재_중국', '태국': '선재_태국'}
...
SONIK_단위      = {'매출액': 1e6, '판매량': 1e3, '영업이익': 1e6,
                   '순금융비용': 1e6, '경상이익': 1e6}
